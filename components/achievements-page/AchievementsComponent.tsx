...
const AchievementsComponent = ({ achievements }: { achievements: AchievementEntity[] }) => {
                ...
                >
                    <Box px={3} py={2} width="100%">
                        <Stack direction="row" alignItems="center" justifyContent="space-around" spacing={2}>
                            ...
                            <DotsHorizontalCircleOutline onClick={(e) => {
                                handleClick();
                            }} />
                        </Stack>
                    </Box>
                    <CardMedia
                        ...
                        {collapse ?
                            <AvatarGroup max={5}>
                                {achievements && achievements.map((achievementEntity: AchievementEntity, i: number) =>
                                    <Avatar alt="AchievementBadge" src={achievementEntity.attributes?.badge?.data?.attributes?.url || ""}></Avatar>)}
                            </AvatarGroup> :
                            <AvatarGroup max={20}>
                                {achievements && achievements.map((achievementEntity: AchievementEntity, i: number) =>
                                    // <WrappedIcon>
