  ...
  useEffect(() => {
    if (socket) {
      socket.on('chat:update', (chatEvent: Chat) => {
        ...
        let updatedHistory = setChatHistory(history);
        console.log('chatEvent socket', chatEvent);
        console.log('updatedHistory socket', updatedHistory);
        console.log('chatHistory socket', chatHistory);
      });
    }
  }, [socket]);

  useEffect(() => {
    console.warn('data in roomId', data);
    if (data) {
